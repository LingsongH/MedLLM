...
try:
    ...
    is_flash_attn_2_available = False
from core.template import get_conv_template
...
def check_and_optimize_memory():
    ...
    logger.info("🔍 检查GPU内存状�?..")

    # 清理缓存
    torch.cuda.empty_cache()

    # 检查每个GPU的内存状�?    num_gpus = torch.cuda.device_count()
    for i in range(num_gpus):
        ...
        logger.info(f"  总内�? {total_memory:.1f}GB")
        logger.info(f"  已分�? {allocated:.1f}GB")
        logger.info(f"  已缓�? {cached:.1f}GB")
        logger.info(f"  可用: {free:.1f}GB")

        if free < 2.0:  # 如果可用内存少于2GB
            logger.warning(f"⚠️ GPU {i} 可用内存不足 ({free:.1f}GB)，建�?")
...
def main():
    ...
    # 设置日志 - 只在主进程输�?    logger.info(f"🚀 使用Accelerate库进行多GPU训练")
    logger.info("🚀 开始初始化Accelerator...")
    # 直接创建Accelerator，让它自己处理状�?    accelerator = Accelerator()
    logger.info("�?Accelerator初始化完�?)
    try:
        logger.info(f"设备: {accelerator.device}")
        logger.info(f"检测到 {accelerator.num_processes} 个进�?)
        logger.info(f"当前进程: {accelerator.process_index}")
        logger.info(f"分布式类�? {accelerator.distributed_type}")
    except:
    ...
    # 检测GPU使用情况并优化内存配�?    total_memory = 0
    if torch.cuda.is_available():
        ...
        for i in range(num_gpus):
            ...
            logger.info(
                f"GPU {i}: 总内�?{gpu_memory:.1f}GB, 已分�?{allocated:.1f}GB, 缓存={cached:.1f}GB, 可用={free:.1f}GB")
    ...
    if is_distributed:
        if script_args.use_tensor_parallel and estimated_model_size_gb > 20:
            ...
            if pkg_resources.parse_version(torch_version) < pkg_resources.parse_version("2.5.0"):
                logger.warning(f"⚠️ 当前PyTorch版本 {torch_version} 不支持张量并行，需�?>= 2.5.0")
    ...
    if torch.cuda.is_available():
        ...
        for i in range(torch.cuda.device_count()):
            ...
            logger.info(f"  GPU {i}: 已分�?{allocated:.1f}GB, 缓存={cached:.1f}GB, 总计={total:.1f}GB")
    ...
    # 设置数据收集�?    data_collator = DataCollatorForSeq2Seq(
        tokenizer=tokenizer,
        model=model,
    ...
    if training_args.do_train:
        ...
        # 计算总训练步�?        num_update_steps_per_epoch = len(train_dataloader) // training_args.gradient_accumulation_steps
    ...
    # 使用Accelerate准备所有组�?- 针对不同并行策略优化
    logger.info("🔄 开始准备训练组�?..")

    # 检查模型是否已经分布在多个设备�?    model_is_distributed = hasattr(model, 'hf_device_map') and model.hf_device_map

    if model_is_distributed:
        logger.info("🔧 检测到模型已分布在多设备，使用兼容模式")
        # 对于已经分布的模型，只准备数据加载器和优化器
        if training_args.do_train:
            # 不要让accelerator包装已经分布的模�?            optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        ...
        # 手动设置模型的训练模�?        model.train() if training_args.do_train else model.eval()

        logger.info("�?分布式模型训练组件准备完�?)
    else:
        logger.info("🔧 标准模式，让Accelerate处理所有组�?)
        if training_args.do_train:
    ...
    # 开始训�?    if training_args.do_train:
        ...
        for epoch in range(int(training_args.num_train_epochs)):
            logger.info(f"开始第 {epoch + 1}/{int(training_args.num_train_epochs)} 轮训�?)

            for step, batch in enumerate(train_dataloader):
                # 针对张量并行优化的训练步�?                if model_is_distributed:
                    # 分布式模型的训练步骤
                    # 前向传播
                    outputs = model(**batch)
                    loss = outputs.loss
                    ...
                    # 梯度累积检�?                    if (step + 1) % training_args.gradient_accumulation_steps == 0:
                        # 梯度裁剪
                ...
                if step_completed:
                    ...
                    if training_args.save_steps > 0 and completed_steps % training_args.save_steps == 0:
                        ...
                        if model_is_distributed:
                            # 分布式模型保�?                            os.makedirs(output_dir, exist_ok=True)
                        ...
                        logger.info(f"保存检查点�? {output_dir}")
        ...
        if accelerator.is_main_process:
            logger.info(f"保存模型�? {training_args.output_dir}")
    ...
    # 最终评�?    if training_args.do_eval and eval_dataloader is not None:
        logger.info("*** 最终评�?***")
        model.eval()
        eval_loss = 0
        eval_steps = 0
        ...
        try:
            ...
            perplexity = float("inf")
        if accelerator.is_main_process:
            logger.info(f"最终评估结�? eval_loss = {avg_eval_loss:.4f}, perplexity = {perplexity:.2f}")
